...
**RLock**类和**locki**类略有不同，它叫**可重入锁(reentrant Lock)**，可重入锁是一种可以由同一个线程多次获取(acquire)的同步原语。也就是说上面例子中要是用RLock类，那么就不会导致死锁。RLock内部维护了一个叫counter的变量，用来记录锁被acquire的次数。如果调用acquire()的线程是资源的所有者，记录调用acquire()次数的计数器就会加1。如果不是，就将试图去获取锁。线程第一次获得锁时，锁的拥有者将会被保存，同时计数器初始化为1。如果调用release方法,首先它会去确认调用者是否是锁的拥有者。如果是的话，计数器减1；如果计数器为0，那么锁将会被释放，这时其他线程就可以去获取锁了。
